...
useEffect(() => {
  ...
  let realName = '';
let saved = localStorage.getItem('ts_tour_' + tourId); // 👈 এটা থাকতেই হবে
...
if (!realName && saved) {
  try {
    ...
    if (parsed?.tour?.name && parsed.tour.name !== 'Tour') {
      realName = parsed.tour.name;
    }
  } catch {}
}
